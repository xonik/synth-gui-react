...
/*
export const createhandlersForController = (
    ctrl: ControllerConfig,
    midiFuncs?: { send?: ParamSendFunc, receive?: ParamReceiveFunc },
    onSetCompleted?: (boundedInput: NumericControllerPayload) => void
): Handler => {
    const set = (input: NumericInputProperty, forceSet = false, uiValue?: number) => {
        ...
        dispatch(setController(boundedInput))
        if(onSetCompleted) onSetCompleted(boundedInput)
...
export const createHandlers = (
    ...
    controllers.forEach((controller) => {
        handlers[controller.id] = new Handler(controller, midiFuncs)
    })
