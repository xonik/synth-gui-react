...
const MixPot = ({ x, y, label, potMode = 'normal', selector, mutator }: {
...
const OutputMixer = ({ x, y, height, width }: ModuleProps) => {
    ...
    return <>
        ...
        <MixPot x={center} y={topPotY} label="Volume"
                selector={s => s.output.volume}
                mutator={(s, v) => { s.output.volume = v }}
        />

        <MixPot x={center} y={topPotY + ROW_HEIGHT} label="Headphones"
                selector={s => s.output.headphones}
                mutator={(s, v) => { s.output.headphones = v }}
        />

        <MixPot x={center} y={topPotY + ROW_HEIGHT * 2} label="Spread" potMode="spread"
        ...
        <MixPot x={center} y={topPotY + ROW_HEIGHT * 4} label="DSP 1"
                selector={s => s.commonFx.fxMix.levelDsp1}
                mutator={(s, v) => { s.commonFx.fxMix.levelDsp1 = v }}
        />

        <MixPot x={center} y={topPotY + ROW_HEIGHT * 5} label="DSP 2"
                selector={s => s.commonFx.fxMix.levelDsp2}
                mutator={(s, v) => { s.commonFx.fxMix.levelDsp2 = v }}
        />

        <MixPot x={center} y={topPotY + ROW_HEIGHT * 6} label="Chorus"
                selector={s => s.commonFx.fxMix.levelChorus}
                mutator={(s, v) => { s.commonFx.fxMix.levelChorus = v }}
        />

        <MixPot x={center} y={topPotY + ROW_HEIGHT * 7} label="Bit crusher"
                selector={s => s.commonFx.fxMix.levelBitCrusher}
                mutator={(s, v) => { s.commonFx.fxMix.levelBitCrusher = v }}
        />
    </>
}
