...
const PreFilterMixerChannel = ({ x, y, label }: ChannelProps) => {
  return <>
    ...
    <RoundPushButton8 x={x+30} y={y}
                      ledPosition="right" ledCount="2" ledOn={[true, false]}
    />
  </>;
};
...
const PreFilterMixer = ({ x, y }: Props) => {
  ...
  const offsetY = 27;
  return <>
    <Header label="Source mix" x={x} y={y} width={80}/>
    ...
    <PreFilterMixerChannel x={x+offsetX} y={y + offsetY + rowDistance * 3} label="Ext aud"/>
    <PreFilterMixerChannel x={x+offsetX} y={y + offsetY + rowDistance * 4} label="Noise"/>
    <PreFilterMixerChannel x={x+offsetX} y={y + offsetY + rowDistance * 5} label="Ring mod"/>
  </>;
};
