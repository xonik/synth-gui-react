...
const Envelope = ({ x, y, label, header, showSelect = false, envId }: Props) => {
    ...
    const bottomRowY = topRowY + ROW_HEIGHT / 1.5
    const potDistance = POT_DISTANCE_L
    ...
    return <>
        ...
        <line x1={x} y1={bottomRowY + 17.5} x2={x+235} y2={bottomRowY + 17.5} className="header-underline" />
        <text
        ...
        >{label}</text>
        <RotaryPot12 ledMode="single" label="Attack" x={firstPotX + potDistance * 0.5} y={topRowY}
                     ctrlGroup={ctrlGroup}
                     ctrl={envCtrls.TIME}
                     ctrlIndex={envId}
                     valueIndex={StageId.ATTACK}
        />
        <RotaryPot12 ledMode="single" label="Decay" x={firstPotX + potDistance * 1.5} y={topRowY}
                     ctrlGroup={ctrlGroup}
                     ctrl={envCtrls.TIME}
                     ctrlIndex={envId}
        ...
        />
        <RotaryPot12 ledMode="multi" label="Sustain" x={firstPotX + potDistance * 2.5 } y={topRowY}
                     ctrlGroup={ctrlGroup}
                     ctrl={envCtrls.LEVEL}
                     ctrlIndex={envId}
        ...
        />
        <RotaryPot12 ledMode="single" label="Release" x={firstPotX + potDistance * 3.5} y={topRowY}
                     ctrlGroup={ctrlGroup}
                     ctrl={envCtrls.TIME}
                     ctrlIndex={envId}
        ...
        <RoundLedPushButton8 label="Loop" x={firstPotX + potDistance * 4.25} y={topRowY} labelPosition="bottom"
                             ctrlGroup={ctrlGroup}
                             ctrl={envCtrls.LOOP}
                             ctrlIndex={envId}
        />
        <RoundLedPushButton8
            label="Invert" x={firstPotX + potDistance * 4.75} y={topRowY}
            labelPosition="bottom"
            ctrlGroup={ctrlGroup}
            ctrl={envCtrls.INVERT}
            ctrlIndex={envId}
        />
        <RoundPushButton8 label="Trigger" x={firstPotX + potDistance * 4.75} y={bottomRowY} labelPosition="bottom"
                          ctrlGroup={ctrlGroup}
