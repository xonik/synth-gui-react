...
const exponentialFunc = (sharpness: number) => {
...
const logarithmicFunc = (sharpness: number) => {
    const a = 1 / (10 ** sharpness - 1)
...
const getScaledPoints = (func: (x: number) => number, yScale: number, keypointCount: number) => {
    const keypoints = getKeypoints(keypointCount)
    const scaled = keypoints.map(func).map((point) => Math.round(yScale * point))
    return scaled
}

const getDerivated = (yValues: number[], intermediatesPerKeypoint: number, accuracy: number) => {
    const derivated = []
